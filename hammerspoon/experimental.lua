  ...
  if role == "AXTextField" or role == "AXTextArea" or role == "AXComboBox" then
    ...
    for _, name in ipairs(currentElement:parameterizedAttributeNames()) do
      logger.i(
        name
          .. ": "
          .. inspect(currentElement:parameterizedAttributeValue(name, {}))
      )
    end
    ...
    logger.i(
      "Children:" .. inspect(currentElement:attributeValue("AXChildren"))
    )
...
hs.hotkey.bind(super, "e", function()
  ...
  local repositionCursor = function()
    ...
    local screenshotOfTextInput =
      hs.screen.mainScreen():snapshot(currentElement:attributeValue("AXFrame"))
    ...
    -- Last visible char
    local visibleRange =
      currentElement:attributeValue("AXVisibleCharacterRange")
...
function hasCurrentSelection()
  local currentApp =
    ax.applicationElement(hs.application.frontmostApplication())
  local menuBar = hs.fnutils.find(
    currentApp:attributeValue("AXChildren"),
  ...
  local editItems =
    editMenu:attributeValue("AXChildren")[1]:attributeValue("AXChildren")
  ...
  processChildren = function(child)
    ...
    if failureCount == 0 then
      ...
      if failureCount == #ax.observer.notifications then
        print(
          string.format("ERROR: All notifications unavailable for: %s", child)
        )
      else
        print(
          string.format(
            "ERROR: Some notifications unavailable for: %s (%s)",
            child,
            failures
          )
        )
...
local appWatcher = hs.application.watcher.new(
  function(applicationName, event, hsApplication)
